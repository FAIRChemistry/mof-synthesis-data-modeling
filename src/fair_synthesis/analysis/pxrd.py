...
def _extract_corresponding_reference(
        pxrd_file: PXRDFile,
        references: list[PXRDFile]) -> PXRDFile | None:
    ...
    """
    _filtered = [f for f in references if f.xray_source ==
                 pxrd_file.xray_source]
    if len(_filtered) > 0:
        references = _filtered
    _filtered = [f for f in references if f.sample_holder_shape ==
                 pxrd_file.sample_holder_shape]
    if len(_filtered) > 0:
        references = _filtered
...
class PXRDPattern(PXRDFile):
    ...
    def __init__(
        ...
        if two_theta is None or intensity is None:
            ...
            #    λ_Cu = 1.540562  # Å
            #    λ_Co = 1.788965  # Å
            # return np.degrees(2 * np.arcsin(λ_Cu / λ_Co *
            # np.sin(np.radians(two_theta) / 2)))
    ...
    def subtract_background(
        ...
        if background is None:
            ...
            def select_background_spectrum(
                    pxrd_file: PXRDFile,
                    background_files: list[str] | None = None) -> PXRDFile:
                if not background_files:
                    _dir = Path(pxrd_file.path).parent
                    _background_files = list(_dir.glob("*PXRD_Blank_*.xyd"))
                    _background_files = [PXRDFile(str(f))
                                         for f in _background_files]
                else:
                ...
                if _background_file is None:
                    raise ValueError(
                        f"No suitable background file found for PXRD file: {
                            pxrd_file.path}")
        ...
        intensity = self.intensity[self.two_theta >=
                                   background.two_theta.min()]
        two_theta = self.two_theta[self.two_theta <=
                                   background.two_theta.max()]
        ...
        return PXRDPattern(
            self,
            two_theta,
            intensity - np.interp(two_theta,
                                  background.two_theta, background.intensity),
        )
    ...
    def normalize(self, settings=None) -> "PXRDPattern":
        ...
        mean_in_range = np.mean(
            self.intensity[(self.two_theta >= reference_range[0])
    ...
    def calc_molar_fraction(self,
                            products: dict[str,
                                           "PXRDPattern"] | dict[str,
                                                                 list["PXRDPattern"]]) -> dict[str,
                                                                                               float]:
        ...
        def _linear_combination_shortening(x, y, components):
            ...
            phase_arrays = [np.asarray(components[name], dtype=float)
            ...
            contributions = {label: value for label,
                             value in zip(labels, weights)}
            contributions["amorphous"] = max(0.0, 1.0 - weights.sum())
            if sum(contributions.values()) > 0:
                total = 1  # sum(contributions.values())
                contributions = {k: v / total for k,
                                 v in contributions.items()}
            return contributions

        _products = {
            k: _extract_corresponding_reference(
                self, v) if isinstance(v, list) else v  # type: ignore
            for k, v in products.items()
        }
        for k, v in _products.items():
            if v is None:
                raise ValueError(
                    f"No suitable product file found for PXRD file: {
                        self.path}")
        ...
        intensities = _linear_combination_shortening(
            ...
            self.intensity,
            {
                k: np.interp(self.two_theta, v.two_theta,
                             v.intensity)  # type: ignore
                for k, v in _products.items()
            },
        )
