...
class ReactionComponent:
    ...
    def from_dict(obj: Any) -> 'ReactionComponent':
        ...
        amount_unit = from_union(
            [AmountUnit, from_none], obj.get("amountUnit"))
        cas_nr = from_union([from_str, from_none], obj.get("casNr"))
        concentration = from_union(
            [from_float, from_none], obj.get("concentration"))
        concentration_unit = from_union(
            [ConcentrationUnit, from_none], obj.get("concentrationUnit"))
        ...
        lab_notebook_entry_and_role = from_union(
            [from_str, from_none], obj.get("labNotebookEntryAndRole"))
        ...
        smiles_stereo = from_union(
            [from_str, from_none], obj.get("smilesStereo"))
        volume = from_union([from_float, from_none], obj.get("volume"))
        volume_unit = from_union(
            [VolumeUnit, from_none], obj.get("volumeUnit"))
    ...
    def to_dict(self) -> dict:
        ...
        if self.amount_unit is not None:
            result["amountUnit"] = from_union(
                [lambda x: to_enum(AmountUnit, x), from_none], self.amount_unit)
        if self.cas_nr is not None:
            result["casNr"] = from_union([from_str, from_none], self.cas_nr)
        if self.concentration is not None:
            result["concentration"] = from_union(
                [to_float, from_none], self.concentration)
        if self.concentration_unit is not None:
            result["concentrationUnit"] = from_union(
                [lambda x: to_enum(ConcentrationUnit, x), from_none], self.concentration_unit)
        if self.density20 is not None:
            result["density20"] = from_union(
                [to_float, from_none], self.density20)
        ...
        if self.inchi_key is not None:
            result["inchiKey"] = from_union(
                [from_str, from_none], self.inchi_key)
        if self.lab_notebook_entry_and_role is not None:
            result["labNotebookEntryAndRole"] = from_union(
                [from_str, from_none], self.lab_notebook_entry_and_role)
        if self.mass is not None:
            result["mass"] = from_union([to_float, from_none], self.mass)
        if self.mass_unit is not None:
            result["massUnit"] = from_union(
                [lambda x: to_enum(MassUnit, x), from_none], self.mass_unit)
        ...
        if self.smiles_stereo is not None:
            result["smilesStereo"] = from_union(
                [from_str, from_none], self.smiles_stereo)
        if self.volume is not None:
            result["volume"] = from_union([to_float, from_none], self.volume)
        if self.volume_unit is not None:
            result["volumeUnit"] = from_union(
                [lambda x: to_enum(VolumeUnit, x), from_none], self.volume_unit)
        return result
...
class Experiment:
    ...
    def from_dict(obj: Any) -> 'Experiment':
        ...
        reaction_components = from_list(
            ReactionComponent.from_dict, obj.get("reactionComponents"))
        reaction_started_when = from_union(
            [from_datetime, from_none], obj.get("reactionStartedWhen"))
        realization_text = from_str(obj.get("realizationText"))
        rinse = from_union([lambda x: from_list(
            from_str, x), from_none], obj.get("rinse"))
        temperature = from_str(obj.get("temperature"))
        temperature_unit = from_union(
            [TemperatureUnit, from_none], obj.get("temperatureUnit"))
        vessel = from_union([Vessel, from_none], obj.get("vessel"))
        wait_after_rinse = from_union(
            [from_int, from_none], obj.get("wait_after_rinse"))
        wait_after_rinse_unit = from_union(
            [Unit, from_none], obj.get("wait_after_rinse_unit"))
    ...
    def to_dict(self) -> dict:
        ...
        if self.degassing is not None:
            result["degassing"] = from_union(
                [lambda x: to_enum(Degassing, x), from_none], self.degassing)
        result["duration"] = from_str(self.duration)
        if self.duration_unit is not None:
            result["durationUnit"] = from_union(
                [lambda x: to_enum(Unit, x), from_none], self.duration_unit)
        if self.evaporate is not None:
            result["evaporate"] = from_union(
                [from_bool, from_none], self.evaporate)
        result["nrInLabJournal"] = from_int(self.nr_in_lab_journal)
        result["observationText"] = from_str(self.observation_text)
        result["reactionComponents"] = from_list(lambda x: to_class(
            ReactionComponent, x), self.reaction_components)
        if self.reaction_started_when is not None:
            result["reactionStartedWhen"] = from_union(
                [lambda x: x.isoformat(), from_none], self.reaction_started_when)
        result["realizationText"] = from_str(self.realization_text)
        if self.rinse is not None:
            result["rinse"] = from_union(
                [lambda x: from_list(from_str, x), from_none], self.rinse)
        result["temperature"] = from_str(self.temperature)
        if self.temperature_unit is not None:
            result["temperatureUnit"] = from_union(
                [lambda x: to_enum(TemperatureUnit, x), from_none], self.temperature_unit)
        if self.vessel is not None:
            result["vessel"] = from_union(
                [lambda x: to_enum(Vessel, x), from_none], self.vessel)
        if self.wait_after_rinse is not None:
            result["wait_after_rinse"] = from_union(
                [from_int, from_none], self.wait_after_rinse)
        if self.wait_after_rinse_unit is not None:
            result["wait_after_rinse_unit"] = from_union(
                [lambda x: to_enum(Unit, x), from_none], self.wait_after_rinse_unit)
        if self.wash_solid is not None:
            result["wash_solid"] = from_union(
                [from_str, from_none], self.wash_solid)
        return result
...
class SciformationCleanedELNSchema:
    ...
    def to_dict(self) -> dict:
        result: dict = {}
        result["experiments"] = from_list(
            lambda x: to_class(Experiment, x), self.experiments)
        return result


def sciformation_cleaned_eln_schema_from_dict(
        s: Any) -> SciformationCleanedELNSchema:
    return SciformationCleanedELNSchema.from_dict(s)


def sciformation_cleaned_eln_schema_to_dict(
        x: SciformationCleanedELNSchema) -> Any:
