...
def convert_cleaned_eln_to_mofsy(eln: SciformationCleanedELNSchema,
    ...
    for experiment in eln.experiments:
        reaction_product = find_reaction_components(
            experiment, RxnRole.PRODUCT)[0]
        reaction_product_mass = format_mass(
            reaction_product.mass, reaction_product.mass_unit)
        reagents: List[ReagentElement] = construct_reagents(
            experiment.reaction_components)
        hardware: Hardware = construct_hardware(experiment)
        procedure: ProcedureSectionsClass = construct_procedure(experiment)
        # pad the experiment nr in lab journal to a length of 3 digits, adding
        # preceding zeros
        experiment_nr = str(experiment.nr_in_lab_journal).zfill(3)
        experiment_id = (
            experiment.code if experiment.code else default_code) + "-" + experiment_nr
...
def construct_procedure(experiment: Experiment) -> ProcedureSectionsClass:
        ...
        if component.rxn_role != RxnRole.PRODUCT:
            prep.append(
                StepEntryClass(
                    ...
                    amount=amount,
                    reagent=component.molecule_name,
                    temp=None,
                    time=None,
                    vessel=vessel,
                    gas=None,
    ...
    procedure = ProcedureSectionsClass(
        prep=prep, reaction=reaction, workup=workup)
    return procedure


def construct_reagents(
        reaction_components: List[ReactionComponent]) -> List[ReagentElement]:
    reagents = []
...
def construct_hardware(experiment: Experiment):
    return Hardware(
        [ComponentElement(id=str(experiment.vessel.value), type=str(
            experiment.vessel.value), chemical=None, comment=None)]
    )
...
def sciformation2mofsy():
    ...
    # Experiment KE-113 had been planned in sciformation but never been
    # carried out, hence it has no specified amounts for the reagents.
    result_dict_procedure['Synthesis'] = [
        entry for entry in result_dict_procedure['Synthesis'] if entry['Metadata']['_description'] != 'KE-113']
