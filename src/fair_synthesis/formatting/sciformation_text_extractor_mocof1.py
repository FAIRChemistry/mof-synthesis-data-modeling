...
def process_data_use_case_specific(data):
    ...
    # Somehow the sciformation export messed up the inchi code for reagend DO.
    # Fix it.
...
def process_realization_text(data):
    ...
    for item in data:
        if 'realizationText' in item:
            ...
            # Else -> Nothing
            solvents = ["acetone", "Et3N", "MeCN",
                        "NaCl aq", "DMF", "CHCl3", "MeOH", "EtOH"]
            for solvent in solvents:
                if solvent.lower() in realization_text.lower() and not any(solvent.lower()
                                                                           in component['moleculeName'].lower() for component in item['reactionComponents']):
            ...
            # If the realizationText contains "samples under fillers" or "MeOH
            # filled up" -> Change "scCO2" into "MeOH+scCO2"
            if any(x in realization_text.lower()
                   for x in ["supercritical co2", "scco<sub>2</sub>", "scco2"]):
                item['wash_solid'] = "scCO2"
            if any(x in realization_text.lower()
                   for x in ["samples under fillers", "meoh filled up"]):
